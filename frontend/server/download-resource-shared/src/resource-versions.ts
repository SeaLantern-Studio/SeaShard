import type { ServerModFilterOption, ServerModVersion } from "@seashard/contracts";
...
export function formatServerModVersionRange(
  ...
  for (let index = 0; index < knownLines.length; index += 1) {
    ...
    if (rangeStart === rangeEnd) ranges.push(first);
    else if (rangeEnd === knownLines.length - 1) ranges.push(`${first}+`);
