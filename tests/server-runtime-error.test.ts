...
await test("runtime errors hide Electron IPC details while preserving actionable messages", () => {
  assert.equal(
    runtimeErrorMessage(
      new Error(
        "Error invoking remote method 'seashard:server-runtime:start': Error: 未检测到 Java 25。NeoForge 26.1 必须使用 Java 25，请先安装或添加对应的 Java 后重试。",
      ),
    ),
    "未检测到 Java 25。NeoForge 26.1 必须使用 Java 25，请先安装或添加对应的 Java 后重试。",
  );
