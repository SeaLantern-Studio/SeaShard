...
await test("runtime errors hide Electron IPC details and classify any missing Java version", () => {
  ...
  const missingJava17Error = new Error(
    "Error invoking remote method 'seashard:server-runtime:start': Error: 未检测到已启用的 Java 17。Pufferfish Purpur 必须使用 Java 17，请先启用、安装或添加对应的 Java 后重试。",
  );
  ...
  assert.equal(isMissingJavaRuntimeError(missingJava25Error), true);
  assert.equal(isMissingJavaRuntimeError(missingJava17Error), true);
  assert.equal(isMissingJavaRuntimeError(new Error("Error: 服务器启动失败")), false);
